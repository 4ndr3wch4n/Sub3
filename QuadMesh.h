...
public:
	...
	void ComputeNormals();
	void createBlob(); //creates the blob
	void moveBlob(float x, float z); //moves the blob
	void change(float height, float width); //scales the blob
	void quadNormal(bool checkR); //changes between quad normal or vertex normal
	void moveBlobMouse(float x, float z); //move blob based on mouse
};
